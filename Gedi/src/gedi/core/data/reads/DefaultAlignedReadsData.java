			...
			for (int i=0; i<count.length; i++) {
				...
				for (int v=0; v<var[i].length; v++) {
					if (data.isMismatch(i, v)) {
						var[i][v] = encodeMismatch(data.getMismatchPos(i, v), data.getMismatchGenomic(i, v).charAt(0), data.getMismatchRead(i, v).charAt(0));
						...
						var[i][v] = encodeInsertion(data.getInsertionPos(i, v), data.getInsertion(i, v));
						indels[i][v] = encodeInsertionIndel(data.getInsertionPos(i, v), data.getInsertion(i, v));
					}
					else if (data.isDeletion(i, v)) {
						var[i][v] = encodeDeletion(data.getDeletionPos(i, v), data.getDeletion(i, v));
						indels[i][v] = encodeDeletionIndel(data.getDeletionPos(i, v), data.getDeletion(i, v));
					} else if (isSoftclip(i, v)) {
						var[i][v] = DefaultAlignedReadsData.encodeSoftclip(getSoftclipPos(i, v), getSoftclip(i, v));
...
//	}
	...
	private static short bytemask = (1<<13)-1;
