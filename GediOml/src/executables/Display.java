...
//					break;
		...
		CpsReader cpsReader = new CpsReader();
		...
		JS js = new JS();
		js.parseParameter(args, false);
		js.putVariable("cps", cpsReader);
		
		InputStream stream = oml==null?Display.class.getResourceAsStream("display.oml.jhp"):new URI(oml.contains("://")?oml:"file://"+new File(oml).getAbsolutePath()).toURL().openStream();
		...
		String cps = new LineIterator(Display.class.getResourceAsStream("/resources/colors.cps")).concat("\n");
		Pipeline pipeline = (Pipeline)new OmlNodeExecutor().addInterceptor(cpsReader.parse(cps)).execute(new OmlReader().setJs(js).parse(src),context);
